...
INFRACTIONS_PERMS_ID = 1348413774528381101
PROMOTIONS_PERMS_ID = 1348413774528381101
BLACKLIST_PERMS_ID = 1348413774528381101
APPEAL_REVIEW_ROLE_ID = 1348413774528381101
INACTIVITY_ROLE_IDS = [1324540074834133033, 1324540189594353787]
INACTIVITY_PERMS_ID = 1348413774528381101

ROLE_HIERARCHY = [
    1348415029837430956, 1348410106135445565, 1348410475225940089, 1348413125866688656,
    1348413276970811435, 1348413355324604456, 1345207151412383804, 1345207152708550697,
    1348409235133698149, 1348409547269738576, 1348410891556880385, 1348474583937974383,
    1306381378446688347, 1345207152029073550
]

def get_active_roles(member, role_ids):
    return [r for r in member.roles if r.id in role_ids]

class SWATManage(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(name="swatmanage")
    async def staff_manage(self, ctx, member: discord.Member):
        ...
        embed = discord.Embed(
            title=f"**{member.display_name}'s** SWAT Panel",
        ...
        try:
            ...
            logger.info(f"SWAT manage embed sent for {member}")
...
class PromotionModal(discord.ui.Modal, title="Promotion"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="📈 SWAT Promotion Notice",
            color=discord.Color.green()
        )
        embed.add_field(name="SWAT Officer", value=self.target.mention, inline=True)
...
class WarningModal(discord.ui.Modal, title="Issue Warning"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"⚠️ SWAT Warning • {role.name}",
            color=discord.Color.red()
        )
        embed.add_field(name="SWAT Officer", value=self.target.mention, inline=True)
...
class TerminationModal(discord.ui.Modal, title="Terminate Staff"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="🚫 SWAT Termination Notice",
            description=f"{self.target.mention} has been terminated from the SWAT.",
            color=discord.Color.red()
        )
        embed.add_field(name="Reason", value=f"> {self.reason.value}", inline=False)
        embed.set_footer(text=f"Issued by {self.moderator.display_name} • {datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
...
async def setup(bot):
    await bot.add_cog(SWATManage(bot))
